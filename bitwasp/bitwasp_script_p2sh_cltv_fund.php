	...
	try {
		...
		foreach(range(1,$_POST['no_of_outputs']) as $this_output) {
			...
			if (!$recipient instanceof Base58AddressInterface) {
				throw new Exception("Invalid P2SH address in 'output#{$this_output}' (Check base58Address).");
			} 
			...
			?>
				<div class="form-row" id='row_output_<?php echo $this_output?>' style="<?php echo ($this_output > $selected_n_outputs) ? "display:none" : "display:;"?>">
					...
					<div class="form-group col-sm-6">
						<input class="form-control" placeholder='P2SH Address' type='text' name='address_<?php echo $this_output?>' value='<?php echo $_POST["address_{$this_output}"]?>'>
					</div>
