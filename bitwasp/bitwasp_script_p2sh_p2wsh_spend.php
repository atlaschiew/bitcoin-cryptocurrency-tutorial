...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	try {
		...
		foreach($utxos as $k=>$utxo) {
			...
			$p2wshScriptPubKey = ScriptFactory::scriptPubKey()->p2wsh($witnessScriptHash/*hash160*/);
			
			//<00 20<32 Bytes Script Hash>>
			$p2shRedeemScript  = ScriptFactory::create()->push($p2wshScriptPubKey->getBuffer())->getScript();
	
			$p2shScriptPubKey = ScriptFactory::scriptPubKey()->p2sh($p2wshScriptPubKey->getScriptHash()/*sha256*/);
			...
			$spendTx = $spendTx->spendOutputFrom($utx, $k,$p2shRedeemScript);
		}
