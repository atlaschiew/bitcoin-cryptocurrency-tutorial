	...
	try {
		...
		foreach(range(1,$_POST['no_of_outputs']) as $this_output) {
			...
			if (!ctype_xdigit($address)) {//hex
				throw new Exception("Error in 'output#{$this_output}'. ScriptPubKey should be hex value.");
			} else {
				...
				if ($decodeScript->getType() != ScriptType::NULLDATA) {
					throw new Exception("Invalid NULL DATA output in 'output#{$this_output}' (Check scriptPubKey).");
				}
			...
			?>
				<div class="form-row" id='row_output_<?php echo $this_output?>' style="<?php echo ($this_output > $selected_n_outputs) ? "display:none" : "display:;"?>">
					...
					<div class="form-group col-sm-6">
						<input class="form-control" title='NULL DATA ScriptPubKey' placeholder='NULL DATA ScriptPubKey' type='text' name='address_<?php echo $this_output?>' value='<?php echo $_POST["address_{$this_output}"]?>'>
					</div>
