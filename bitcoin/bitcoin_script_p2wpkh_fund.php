	...
	try {
		...
		foreach(range(1,$_POST['no_of_outputs']) as $thisOutput) {
			...
			if (!$recipient instanceof Bech32AddressInterface) {
				throw new Exception("Invalid P2WPKH address in 'output#{$thisOutput}' (Check bech32Address).");
			} 
			
			$decodeScript = (new OutputClassifier())->decode($recipient->getScriptPubKey());
			
			if ($decodeScript->getType() != ScriptType::P2WKH) {
				throw new Exception("Invalid P2WPKH address in 'output#{$thisOutput}' (Check scriptPubKey).");
			}
			...
			?>
				<div class="form-row" id='row_output_<?php echo $thisOutput?>' style="<?php echo ($thisOutput > $selectedNOutputs) ? "display:none" : "display:;"?>">
					...
					<div class="form-group col-sm-6">
						<input class="form-control" placeholder='P2WPKH Address' type='text' name='address_<?php echo $thisOutput?>' value='<?php echo $_POST["address_{$thisOutput}"]?>'>
					</div>
